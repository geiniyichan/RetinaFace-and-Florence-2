...
def crop_faces(original_image_path, detections, output_folder):
    """
    裁剪人脸图片
    :param original_image_path: 原始未标注图片路径
    :param detections: 检测结果，格式为 [x1, y1, x2, y2, score]
    :param output_folder: 裁剪后的人脸图片保存路径
    """
    # 读取原始未标注图片
    image = cv2.imread(original_image_path)
    if image is None:
        print(f"无法读取图片: {original_image_path}")
        return
    ...
    for i, det in enumerate(detections):
        x1, y1, x2, y2 = map(int, det[:4])  # 提取边框坐标
        ...
        if x2 > x1 and y2 > y1:
            face = image[y1:y2, x1:x2]  # 裁剪人脸区域
            
            # 保存裁剪后的人脸图片
            face_path = os.path.join(output_folder, f"face_{i}.jpg")
            cv2.imwrite(face_path, face)
        else:
            print(f"无效的人脸边框坐标: {x1}, {y1}, {x2}, {y2}")
...
if __name__ == '__main__':
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        if args.save_image:
            ...
            # 裁剪人脸图片
            output_folder = "./faces/"
            crop_faces(image_path, dets, output_folder)
